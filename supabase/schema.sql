...
-- Redacción (logueada con Supabase Auth): acceso total
...
-- ------------------------------------------------------------
-- Imágenes de las notas (lectura pública)
...
-- Word originales que manda la gente (privado)
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
...
create policy notas_img_editorial_write on storage.objects
  for insert to authenticated with check (bucket_id = 'notas-img');
...
create policy notas_img_editorial_update on storage.objects
  for update to authenticated using (bucket_id = 'notas-img');
...
create policy notas_img_editorial_delete on storage.objects
  for delete to authenticated using (bucket_id = 'notas-img');
...
create policy notas_docs_editorial_read on storage.objects
  for select to authenticated using (bucket_id = 'notas-docs');
...
create policy notas_docs_editorial_delete on storage.objects
  for delete to authenticated using (bucket_id = 'notas-docs');

-- ------------------------------------------------------------
--  (opcional) una nota de ejemplo ya publicada, para ver el diseño
-- ------------------------------------------------------------
-- insert into public.notas (titulo, subtitulo, bajada, cuerpo_html, autor, email_autor,
--                           centros, estado, destacada, fecha_publicacion)
-- values (
--   'Una nota de ejemplo', 'El subtítulo va acá',
--   'Un resumen corto de dos o tres líneas que se ve en la tarjeta y arriba de la nota.',
--   '<p>Primer párrafo del cuerpo. Se guarda como HTML y se muestra formateado.</p><h2>Un intertítulo</h2><p>Otro párrafo.</p>',
--   'Equipo Cíclica', 'redaccion@estudiosrenovacion.com',
--   array['CEER'], 'publicada', true, current_date
-- );
